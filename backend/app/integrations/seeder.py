...
class DatabaseSeeder:
    ...
    def seed_all(self, limit: Optional[int] = None):
        """
        ...
        # First pass: Create commodities and mandis
        self._seed_commodities(records)
        self._seed_mandis(records)
        
        # Second pass: Create price history
    ...
    def _seed_commodities(self, records: list[dict]):
        ...
        for name in sorted(commodity_names):
            existing = self.db.query(Commodity).filter(
                Commodity.name == name
            ).first()
            
            if existing:
                self._commodity_cache[name] = existing
                continue
    ...
    def _seed_mandis(self, records: list[dict]):
        ...
        for key, data in mandis_data.items():
            existing = self.db.query(Mandi).filter(
                ...
                Mandi.district == data["district"],
                Mandi.state == data["state"],
            ).first()
            
            if existing:
                self._mandi_cache[key] = existing
                continue
