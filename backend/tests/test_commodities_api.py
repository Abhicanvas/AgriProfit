...
def test_list_commodities(client, test_commodity):
    ...
    response = client.get("/commodities/?skip=0&limit=10")
    assert response.status_code == 200
    data = response.json()
...
def test_filter_commodities_by_category(client, test_commodity):
    ...
    response = client.get(f"/commodities/?category={test_commodity.category}")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
...
def test_update_commodity(client, test_admin_token, test_commodity):
    ...
    response = client.put(f"/commodities/{test_commodity.id}", json=update_data, headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == str(test_commodity.id)
