...
def test_create_post(client, test_token, test_user):
    ...
    response = client.post("/community/posts/", json=data, headers=headers)
...
def test_create_post_unauthorized(client):
    ...
    response = client.post("/community/posts/", json=data)
    assert response.status_code == 401

def test_get_post_by_id(client, test_post):
    """Test GET /community/posts/{id}."""
    response = client.get(f"/community/posts/{test_post.id}")
...
def test_list_posts(client, test_post):
    ...
    response = client.get("/community/posts/?skip=0&limit=10")
    assert response.status_code == 200
    data = response.json()
...
def test_filter_posts_by_author(client, test_post, test_user):
    ...
    response = client.get(f"/community/posts/?user_id={test_user.id}")
    assert response.status_code == 200
    data = response.json()
    assert "items" in data
    assert isinstance(data["items"], list)
...
def test_update_post_success(client, test_token, test_post, test_user):
    ...
    response = client.put(f"/community/posts/{test_post.id}", json=update_data, headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == str(test_post.id)
...
def test_delete_post_success(client, test_token, test_post, test_user):
    ...
    headers = get_auth_headers(test_token)
    response = client.delete(f"/community/posts/{test_post.id}", headers=headers)
