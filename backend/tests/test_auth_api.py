...
def test_request_otp_success(client, test_db, phone_number):
    ...
    response = client.post("/auth/request-otp", json={"phone_number": phone_number})
...
def test_request_otp_invalid_phone(client):
    response = client.post("/auth/request-otp", json={"phone_number": "invalid"})
...
def test_verify_otp_success(client, test_db, phone_number, otp):
    ...
    response = client.post("/auth/verify-otp", json={"phone_number": phone_number, "otp": otp})
...
def test_verify_otp_invalid(client, test_db, phone_number, otp):
    ...
    test_db.refresh(user)
    # Try wrong OTP
    response = client.post("/auth/verify-otp", json={"phone_number": phone_number, "otp": "000000"})
