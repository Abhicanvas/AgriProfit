...
def test_get_price_trends(client, test_commodity, test_mandi, test_prices):
    ...
    response = client.get(f"/analytics/trends/{test_commodity.id}")
...
def test_get_price_trends_with_filters(client, test_commodity, test_mandi, test_prices):
    ...
    response = client.get(
        f"/analytics/trends/{test_commodity.id}?mandi_id={test_mandi.id}&days=30"
    )
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
...
def test_get_price_statistics(client, test_commodity, test_mandi, test_prices):
    ...
    response = client.get(f"/analytics/statistics/{test_commodity.id}")
    assert response.status_code == 200
    data = response.json()
...
def test_get_top_commodities(client, test_prices):
    ...
    response = client.get("/analytics/top-commodities?limit=10&days=30")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
...
def test_get_user_activity(client, test_token, test_user, test_post):
    ...
    response = client.get(f"/analytics/user-activity/{test_user.id}", headers=headers)
    assert response.status_code == 200
    data = response.json()
...
def test_get_user_activity_unauthorized(client, test_user):
    ...
    response = client.get(f"/analytics/user-activity/{test_user.id}")
