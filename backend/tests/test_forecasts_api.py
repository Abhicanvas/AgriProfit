...
def test_create_forecast(client, test_admin_token, test_commodity, test_mandi):
    ...
    response = client.post("/forecasts/", json=data, headers=headers)
...
def test_create_forecast_unauthorized(client, test_commodity, test_mandi):
    ...
    response = client.post("/forecasts/", json=data)
    assert response.status_code == 401

def test_get_forecast_by_id(client, test_forecast):
    """Test GET /forecasts/{id}."""
    response = client.get(f"/forecasts/{test_forecast.id}")
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == str(test_forecast.id)
    assert data["predicted_price"] == test_forecast.predicted_price

def test_list_forecasts(client, test_forecast):
    """Test GET /forecasts/ with pagination."""
    response = client.get("/forecasts/?skip=0&limit=10")
    assert response.status_code == 200
    data = response.json()
    assert "items" in data
    assert isinstance(data["items"], list)
    assert any(f["id"] == str(test_forecast.id) for f in data["items"])

def test_filter_forecasts_by_commodity(client, test_forecast):
    """Test GET /forecasts/?commodity_id=X."""
    response = client.get(f"/forecasts/?commodity_id={test_forecast.commodity_id}")
    assert response.status_code == 200
    data = response.json()
    assert "items" in data
    assert any(f["commodity_id"] == str(test_forecast.commodity_id) for f in data["items"])

def test_filter_forecasts_by_date_range(client, test_db, test_commodity, test_mandi):
    """Test GET /forecasts/?start_date=X&end_date=Y."""
    ...
    data = response.json()
    assert "items" in data
    assert any(f["id"] == str(forecast2.id) for f in data["items"])
    assert all(start <= date.fromisoformat(f["forecast_date"]) <= end for f in data["items"])
...
def test_get_latest_forecast(client, test_commodity, test_mandi, test_forecast):
    ...
    response = client.get(f"/forecasts/latest?commodity_id={test_commodity.id}&mandi_id={test_mandi.id}")
    assert response.status_code == 200
    data = response.json()
...
def test_update_forecast(client, test_admin_token, test_forecast):
    ...
    response = client.put(f"/forecasts/{test_forecast.id}", json=update_data, headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == str(test_forecast.id)
...
def test_delete_forecast(client, test_admin_token, test_forecast):
    ...
    headers = get_auth_headers(test_admin_token)
    response = client.delete(f"/forecasts/{test_forecast.id}", headers=headers)
