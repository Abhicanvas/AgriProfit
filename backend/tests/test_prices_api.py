...
def test_create_price_history(client, test_admin_token, test_commodity, test_mandi):
    ...
    response = client.post("/prices/", json=data, headers=headers)
...
def test_create_price_unauthorized(client, test_commodity, test_mandi):
    ...
    response = client.post("/prices/", json=data)
    assert response.status_code == 401

def test_get_price_by_id(client, test_price):
    """Test GET /prices/{id}."""
    response = client.get(f"/prices/{test_price.id}")
...
def test_list_prices(client, test_price):
    ...
    response = client.get("/prices/?skip=0&limit=10")
    assert response.status_code == 200
    data = response.json()
...
def test_filter_prices_by_commodity(client, test_price):
    ...
    response = client.get(f"/prices/?commodity_id={test_price.commodity_id}")
    assert response.status_code == 200
    data = response.json()
    assert "items" in data
    assert isinstance(data["items"], list)
...
def test_filter_prices_by_date_range(client, test_db, test_commodity, test_mandi):
    ...
    response = client.get(f"/prices/?start_date={start}&end_date={end}")
    assert response.status_code == 200
    data = response.json()
    assert "items" in data
    assert isinstance(data["items"], list)
...
def test_get_latest_price(client, test_db, test_commodity, test_mandi):
    ...
    test_db.add(price2)
    test_db.commit()
    response = client.get(f"/prices/latest?commodity_id={test_commodity.id}&mandi_id={test_mandi.id}")
    assert response.status_code == 200
    data = response.json()
...
def test_update_price(client, test_admin_token, test_price):
    ...
    response = client.put(f"/prices/{test_price.id}", json=update_data, headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == str(test_price.id)
...
def test_delete_price(client, test_admin_token, test_price):
    ...
    headers = get_auth_headers(test_admin_token)
    response = client.delete(f"/prices/{test_price.id}", headers=headers)
