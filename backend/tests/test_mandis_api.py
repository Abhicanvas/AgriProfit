...
def test_create_mandi_success(client, test_admin_token):
    ...
    response = client.post("/mandis/", json=data, headers=headers)
...
def test_create_mandi_unauthorized(client):
    ...
    response = client.post("/mandis/", json=data)
    assert response.status_code == 401

def test_get_mandi_by_id(client, test_mandi):
    """Test GET /mandis/{id}."""
    response = client.get(f"/mandis/{test_mandi.id}")
...
def test_list_mandis(client, test_mandi):
    ...
    response = client.get("/mandis/?skip=0&limit=10")
    assert response.status_code == 200
    data = response.json()
...
def test_filter_mandis_by_state(client, test_mandi):
    ...
    response = client.get(f"/mandis/?state={test_mandi.state}")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
...
def test_filter_mandis_by_district(client, test_mandi):
    ...
    response = client.get(f"/mandis/?district={test_mandi.district}")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
...
def test_update_mandi(client, test_admin_token, test_mandi):
    ...
    response = client.put(f"/mandis/{test_mandi.id}", json=update_data, headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["id"] == str(test_mandi.id)
...
def test_delete_mandi(client, test_admin_token, test_mandi):
    ...
    headers = get_auth_headers(test_admin_token)
    response = client.delete(f"/mandis/{test_mandi.id}", headers=headers)
